...
namespace Visualizer
{
    ...
    {
        [Header("Players")] [SerializeField] private GameObject quarterback;
        [SerializeField] private GameObject wideReceiver;
        [SerializeField] private GameObject runningBack;
        [Header("Football")] [SerializeField] private GameObject football;

        [Header("Switch POV")] [SerializeField]
        private List<SwitchPOV> switchPOV;

        [SerializeField] private GameObject locomotionSystem;
        [Header("UI Menu")] [SerializeField] private GameObject menu;
        [SerializeField] private List<TextMeshPro> loadedPlayName;
        [SerializeField] private TextMeshPro durationPlayTime;
        ...
        private Vector3 _initialRefPosition;
        private Quaternion _initialRefRotation;
        private PlayData _playData;
        private int _povIndex;
        private float _playbackSpeed = 1.5f;
        private Transform _player;

        public Transform CurrentPlayer => _player;

        private void Start()
        {
            // Set Current player
            _player = switchPOV[0].gameObject.transform;

            // Reset REF position and rotation
            _initialRefPosition = switchPOV[0].transform.position;
            _initialRefRotation = switchPOV[0].transform.rotation;
        }
        ...
        public void LoadPlayData(int playFile)
        {
            ...
            if (playDataFile != string.Empty)
            {
                ...
                _playData = JsonUtility.FromJson<PlayData>(playJson.text);

                Debug.Log($"Loaded play: {_playData.metadata.playName}");
            }
        }
        ...
        public void ReplayPlay()
        {
            ...
            if (_povIndex != 0)
            {
                foreach (var controller in switchPOV[_povIndex].Controllers)
                {
            ...
            StartCoroutine(PlaybackMovement(_playData));
        }
        ...
        private void SetXRPov()
        {
            for (int i = 0; i < switchPOV.Count; i++)
            {
                ...
                {
                    // Set Current player
                    _player = switchPOV[i].gameObject.transform;
        ...
        private IEnumerator PlaybackMovement(PlayData play)
        {
            ...
            if (_povIndex != 0)
            {
                ...
                if (_initialRefPosition != switchPOV[0].transform.position ||
                    _initialRefRotation != switchPOV[0].transform.rotation)
                {
                    switchPOV[0].transform.SetPositionAndRotation(_initialRefPosition, _initialRefRotation);
                }
            }
        ...
        private IEnumerator PlayEnded()
        {
            ...
            menu.transform.position = _player.TransformPoint(menuOffset);
            // Rot: have menu face the same direction as player
            menu.transform.rotation = Quaternion.Euler(0, _player.eulerAngles.y, 0);

            durationPlayTime.text = $"{_playData.metadata.duration}seconds";
            menu.SetActive(true);

            // Turn XR Controllers ON
            var currPlayerControllers = _player.GetComponent<SwitchPOV>().Controllers;
